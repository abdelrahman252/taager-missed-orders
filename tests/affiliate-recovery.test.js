...
const easyOrdersExportDialogSource = fs.readFileSync(path.join(root, "src", "bot", "easy-orders-export-dialog.js"), "utf8");
...
assert(easyOrdersExportSource.includes("clickExportButton(page, exportButton, keyword)") && easyOrdersExportSource.includes("EASY_ORDERS_EXPORT_BUTTON_UNAVAILABLE"), "EasyOrders export button should use bounded retries instead of an unbounded click");
assert(easyOrdersExportDialogSource.includes("EASY_ORDERS_EXPORT_DIALOG_NOT_OPEN"), "EasyOrders export should report when the dialog does not open after the button click");
...
assert(failedFlowSource.includes('"/orders/failed-orders"'), "failed-orders diagnosis should open the current Taager failed-orders route");
assert(failedFlowSource.includes("#failed-orders-date-pill"), "failed-orders diagnosis should use the current failed-orders date filter");
assert(failedFlowSource.includes("#failed-orders-date-apply"), "failed-orders diagnosis should apply the current failed-orders date range");
assert(failedFlowSource.includes("#failed-orders-export-excel-button"), "failed-orders diagnosis should use the current Taager Excel export button id");
assert(failedFlowSource.includes('button[aria-label="${aria.replace'), "failed-orders diagnosis should support current localized calendar day labels");
assert(failedFlowSource.includes("clickCalendarControl") && failedFlowSource.includes("scrollIntoViewIfNeeded"), "failed-orders calendar controls should use bounded scroll/retry clicks");
assert(failedFlowSource.includes("selected exact date range"), "failed-orders diagnosis should preserve both requested date bounds");
assert(easyOrdersExportSource.includes("clickExportDialogSubmit(page, dialog, keyword)") && easyOrdersExportDialogSource.includes("EASY_ORDERS_EXPORT_SUBMIT_UNAVAILABLE"), "EasyOrders export submit should use the dedicated dialog submit control");
assert(easyOrdersExportDialogSource.includes('getByRole("dialog", { name: EXPORT_DIALOG_NAME })') && easyOrdersExportDialogSource.includes('EXPORT_DIALOG_NAME = /^Export Orders$/i'), "EasyOrders export should select its modal by title rather than the first visible dialog");
assert(easyOrdersExportDialogSource.includes('getByRole("button", { name: EXPORT_SUBMIT_NAME })') && easyOrdersExportDialogSource.includes('EXPORT_SUBMIT_NAME = /^Export$/i'), "EasyOrders export should click only the exact Export action inside its named modal");
assert(easyOrdersExportSource.includes("closeExportDatePicker(page, keyword)"), "EasyOrders export should wait for the date picker to close before locating modal actions");
...
assert(resultsSource.includes("emptyQueue") && resultsSource.includes("recovery_zero_queue_title"), "zero eligible recovery orders should be shown explicitly instead of displaying an all-clear message");
assert(appSource.includes("results.recovery_zero_queue_body") && appSource.match(/results\.recovery_zero_queue_body/g).length >= 2, "zero-queue explanation should be translated for English and Arabic results");
...
assert(resultsSource.includes("customerIdentityDetailFor") && resultsSource.includes("identityIssueLabels"), "manual-review rows should expose the exact stored customer identity validation issue");
assert(resultsSource.includes("manual-missing-value") && resultsSource.includes("Missing SKU") && resultsSource.includes("Missing quantity"), "manual-review rows should label missing source, SKU, and quantity values instead of ambiguous dashes");
assert(resultsSource.includes("diagnostics_col"), "manual-review downloads should include validation and data-completeness diagnostics");
