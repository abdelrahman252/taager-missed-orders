...
async function verifyDashboardPerformanceAcceptance(page) {
  ...
  if (after.dashboardPaneChildren > 5) throw new Error("Dashboard pane cache is unbounded: " + JSON.stringify({ before, after }));
...
async function verifyProductReactiveInteractions(page) {
  ...
  const persisted = await page.evaluate(() => window.DashboardRoiState.get("__all__"));
  if (persisted.currency !== "USD" || persisted.adSpend !== 1200) {
  ...
  if (forecastPublished.currency !== "EGP" || forecastPublished.adSpend !== 1200) {
  ...
  if (!resetForecastSort.clearDisabled || resetForecastSort.firstRow !== resetForecastSort.initialRow) {
  ...
  if (!productValues.rawFields || !productValues.usdFields || Math.abs(productValues.spendTotal - 1200) > 0.06) {
