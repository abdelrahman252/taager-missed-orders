...
function renderSectionCitiesHydrated(mountEl, data, ctx) {
  ...
  if (typeof window.animateNumber === "function") {
    mountEl.querySelectorAll("[data-animate-num]").forEach(function (el) {
      ...
      if (!isNaN(to)) window.animateNumber(el, to, { duration: 0 });
    });
  }
...
window.renderSectionCitiesHydratedEntry = function (mountEl, data, ctx) {
  if (!mountEl) return;
  ...
  var shouldDefer = !window._dashboardInteractiveSectionsReady &&
